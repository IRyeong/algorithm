...
public class Main {
	...
	static long fw[][][]; // 0 : 플로이드 워셜을 이용해 구한 꼭짓점부터 꼭짓점까지 가는데 걸리는 시간, 1 : 놀리는 최대 시간
	...
	public static void main(String[] args) throws Exception {
		...
		Arrays.sort(teasing);
//		for (int i = 0; i <= N; i++) {
//			System.out.println(teasing[i].num+", "+teasing[i].time);
//		}
		// 플로이드 워셜로 거리 모두 구해두기
		for (Pair p : teasing) {
			if(p.num==0) {
			...
			for (int i = 1; i <= N; i++) {
				for (int j = 1; j <= N; j++) {
					long tmpMax = max(fw[i][p.num][1], fw[p.num][j][1], p.time);
	...
	private static long max(long a, long b, int c) {
		long ret;
		ret = Math.max(a, b);
		ret = Math.max(ret, c);
		return ret;
